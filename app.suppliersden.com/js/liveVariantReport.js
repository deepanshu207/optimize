// liveVariantReport.js — non-module version bundled as content script
// Auto-generated from liveVariantReport.mjs; do not edit this file directly.
(function(){
"use strict";
/**
...
function pickBestAtPrice(variants, price) {
...
function uniqueSortedPrices(variants) {
 ...
 */
function findLowestRupeePair(prices) {
...
function findAllRupeePairs(prices) {
  const pairs = [];
  for (let i = 0; i < prices.length - 1; i++) {
    if (prices[i + 1] - prices[i] === 1) {
...
function pickRecommendedVariants(variants) {
 ...
 */
function analyzeLiveVariants(variants, context = {}) {
...
function exportReportTxt(analysis) {
  const ctx = analysis.context || {};
  const rec = analysis.recommendation || {};
  const stats = analysis.stats || {};
  const lines = [];
...
function buildReportFilename(analysis, ext) {
...
function downloadReportBlob(content, filename, mime) {
 ...
 */
function createAndDownloadReport(variants, context = {}) {
